...
// ─── Supabase Storage ────────────────────────────────────────────────────────
export const uploadToSupabase = async (fileOrBlob, bucket = 'study-notes') => {
    // 1. Generate a unique filename
    const fileExt = fileOrBlob.name ? fileOrBlob.name.split('.').pop() : 'bin';
...
export const uploadMedia = async (params) => {
    ...
    if (fileSource) {
        try {
            ...
            params.drive_link = url;    // Store the public URL
            
            // Clean up heavy properties
            delete params.base64data;
            delete params.file;
        } catch (err) {
            console.error('Supabase Storage upload failed:', err);
            
            // Fallback to Google Drive if bucket isn't ready or other error
            console.log('Attempting fallback to Google Drive...');
            try {
                const { uploadFileToDrive } = await import('./googleAuth');
                const STUDY_NOTES_FOLDER = '1hWZY9Uk-0u5pgUOEYVER91IPdReEVxuk';
                ...
                console.error('All upload methods failed:', driveErr);
                throw driveErr;
            }
        }
    }
